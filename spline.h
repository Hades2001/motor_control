...
bool spline::make_monotonic()
{
    ...
    assert(m_x.size()>2);
    bool modified = false;
    const uint16_t n = uint16_t(m_x.size());
    // make sure: input data monotonic increasing --> b_i>=0
    //            input data monotonic decreasing --> b_i<=0
    for(uint16_t i=0; i<n; i++) {
        uint16_t im1 = uint16_t(std::max(i-1, 0));
        uint16_t ip1 = uint16_t(std::min(i+1, n-1));
...
size_t spline::find_closest(double x) const
{
    ...
    size_t idx = size_t(std::max( int(it-m_x.begin())-1, 0));   // m_x[idx] <= x
    return idx;
}
...
void band_matrix::resize(int dim, int n_u, int n_l)
{
    ...
    assert(n_u>=0);
    assert(n_l>=0);
    m_upper.resize(uint16_t(n_u+1));
    m_lower.resize(uint16_t(n_l+1));
    for(size_t i=0; i<m_upper.size(); i++) {
        m_upper[i].resize(uint16_t(dim));
    }
    for(size_t i=0; i<m_lower.size(); i++) {
        m_lower[i].resize(uint16_t(dim));
    }
}
...
double & band_matrix::operator () (int i, int j)
{
    ...
    if(k>=0)    return m_upper[uint16_t(k)][uint16_t(i)];
    else        return m_lower[uint16_t(-k)][uint16_t(i)];
}
...
std::vector<double> band_matrix::l_solve(const std::vector<double>& b) const
{
    ...
    std::vector<double> x(uint16_t(this->dim()));
    int j_start;
    double sum;
    for(int i=0; i<this->dim(); i++) {
        sum=0;
        j_start=std::max(0,i-this->num_lower());
        for(int j=j_start; j<i; j++) sum += this->operator()(i,j)*x[uint16_t(j)];
        x[uint16_t(i)]=(b[uint16_t(i)]*this->saved_diag(i)) - sum;
    }
    return x;
}
// solves Rx=y
std::vector<double> band_matrix::r_solve(const std::vector<double>& b) const
{
    ...
    for(int i=this->dim()-1; i>=0; i--) {
        ...
        for(int j=i+1; j<=j_stop; j++) sum += this->operator()(i,j)*x[uint16_t(j)];
        x[uint16_t(i)]=( b[uint16_t(i)] - sum ) / this->operator()(i,i);
    }
    return x;
}
